...
def get_path():
    ...
    for i in range(len(texts)):
        points = texts[i][4:-4].split(',')  # 结尾少读取两个
        match_flag=texts[i].split(',')[-1]
        flag_list.append(int(match_flag))
        for point in points:
            point_list[i].append(int(point))
    # print point_list
    return point_list,flag_list
...
for day in range(20,28):
    ...
    infile = 'method3_out'+date+'.txt'
    atime_list, pnum_list = get_info()
    point_list,flag_list = get_path()
    print 'the number of vehicle',len(flag_list)
    ...
    for i in range(lenpath):
        ...
        if person_per_car[i]==4 or flag_list[i]==2 or flag_list[i]==1:
    ...
    tot_waiting_time=sum(wating_time_list)
    ...
    f=open(out_file,'a')
    f.write(outstring+'\ntot_waiting time'+str(tot_waiting_time))
    f.close()
    outfile2 = '../ppc.csv'
    f=open (outfile2,'a')
